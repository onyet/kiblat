...
Future<void> showExitAndMaybeShowAd(BuildContext context,
    {Duration adTimeout = const Duration(seconds: 4)}) async {
      ...
      return StatefulBuilder(builder: (ctx2, setState) {
        return AlertDialog(
          ...
          actions: isLoading
              ...
              : [
                  TextButton(
                    onPressed: () => Navigator.of(ctx2, rootNavigator: true).pop(false),
                    child: Text(tr('stay')),
                  ),
                  TextButton(
                    onPressed: () async {
                      ...
                      timer = Timer.periodic(const Duration(milliseconds: 50), (t) {
                        ...
                        final double normalized = math.min(1.0, elapsed / totalMs);
                      ...
                      TelemetryService.instance
                          .trackExitAttempt(confirmed: true);

                      bool adShown = false;
                      try {
                        adShown = await AdService.instance
                            .showInterstitialAd(timeout: adTimeout);
                        TelemetryService.instance.trackAdResult(shown: adShown);
                      } catch (e) {
                        TelemetryService.instance
                            .logEvent('exit_ad_error', {'error': e.toString()});
                      }
